...
namespace cppflow {
    ...
    public:
        ...
        std::string get_default_operation() const;
...
namespace cppflow {
    ...
    inline std::string model::get_default_operation() const {
        for (auto& op : get_operations()) {
            if (op.find("serving_default") != std::string::npos) {
                return op;
            }
        }
        return "No default op found";
    }
    ...
    inline tensor model::operator()(const tensor& input) {
        return (*this)({{get_default_operation(), input}}, {"StatefulPartitionedCall"})[0];
    }
}
